...
fs_timing = [0; 50; 100; 150; 200; 250; 300; 350; 400; 450; 500; 550; 600; 650; 700; 750; 800];
...
for j = 1:sim_duration_video
    ...
    % do not care for now
    if fsCounter == 1 && ct >= 1 && ct < 15
       bang_x = mpcTimeStep*0;   
       bang_y = mpcTimeStep*1;  %guess m/s da quantificare in N
    else
       bang_x = 0; 
       bang_y = 0;       
    end 
...
titleee = title("Trotting: $\phi$ = $0$, step = $0.10$ [m]","Interpreter","Latex");
titleee.FontSize = 20;
% clf
hold on
grid on  
...
axis([-0.2 2.5 -0.3 0.3001]); % phi = 0
pbaspect([4 2 1]); % phi = 0

% axis([-0.2 1 -0.3 1.5]); % phi = pi/2
% pbaspect([2 4 1]); % phi = pi/2

% axis([-0.2 2 -0.3 1]); % phi = pi/4
% pbaspect([4 3 1]); % phi = pi/4
...
% writerObj = VideoWriter('Trotting_phipi2_15cm.avi');
