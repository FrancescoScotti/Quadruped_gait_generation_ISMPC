...
inline Eigen::VectorXd solveQP_hpipm(Eigen::MatrixXd costFunctionH, Eigen::VectorXd costFunctionF, 
    ...
    int n_constr = AConstraint.rows() - 2;

    int nv = n_variables;
    int ne = 2;  //number of equality constraints
    //if (itr == 20 ) ne = 0;
    ...
    for (int k = 0; k<n_constr; k++) {
        d_lb[k] = bConstraintMin(k+2); //bConstraintMin(k);
        d_ub[k] = bConstraintMax(k+2); //bConstraintMax(k);

        for (int j = 0; j<n_variables; j++) {
            C[j*n_constr+k] = AConstraint(k+2,j); //AConstraint(k,j);
        }
    }
    ...
    for (int k = 0; k<2; k++) {
        ...
        for (int j = 0; j<n_variables; j++) {
            A[j*2+k] = AConstraint(k,j); //Aeq(k,j);
        }
    }
